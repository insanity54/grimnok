...
class Nakama {
  ...
  async authenticate(nip98TokenValue: string): Promise<void> {
    this.client = new Client('defaultkey', 'localhost', '7350');
    

    // let deviceId: string | null = localStorage.getItem('deviceId');
    // let nostrPubkey: string | null = await window.nostr.getPublicKey();
    // let nip98Token: string | null = '';

    // if (!deviceId) {
    //   deviceId = uuidv4();
    //   localStorage.setItem('deviceId', deviceId);
    // }
    if (!nip98TokenValue) throw new Error('arg0 nip98TokenValue was missing');
